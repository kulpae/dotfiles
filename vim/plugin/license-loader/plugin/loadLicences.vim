...
let s:root_path=expand("<sfile>:h:h")
"the function to load License
function! LoadMyLicense(license)
	silent! :execute "1"
	silent! :execute '0r '.s:root_path.'/templates/license/'.a:license.'.license'
	...
	if exists("g:T_YEAR_FORMAT")
		sil! execute 1.','."$"."s/<+T_YEAR+>/".strftime(g:T_YEAR_FORMAT)."/g"
	endif
endfunction
